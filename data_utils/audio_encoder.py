...
from lib.naturalspeech3_facodec.ns3_codec.facodec import (
    FACodecEncoderV2,
    FACodecDecoderV2,
)
...
class BaseAudioPreprocessor:
    ...
    def resample(
        self, wav: np.ndarray, orig_sr: int, target_sr: Optional[int] = None
    ) -> np.ndarray:
    ...
    def preprocess(
        self, path_or_wav: Union[str, np.ndarray], sr: Optional[int] = None
...
class FACodecEncoder:
    ...
    def encode(self, wav: str | bytes | list[str | bytes], sr: int = 16000):
        ...
        padded = [
            torch.nn.functional.pad(t, (0, max_len - t.shape[0])) for t in audio_tensors
        ]
